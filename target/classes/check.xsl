<xsl:stylesheet
	...
	<xsl:template match="checkstyle" mode="filelist">
		...
		<table class="log" border="0" cellpadding="5" cellspacing="2"
			...
			<xsl:for-each
				...
				<!-- Sort method 1: Primary by #error, secondary by #warning, tertiary 
					by #info -->
				...
				<!-- Sort method 1: Sum(#error+#info+#warning) (uncomment to use, comment 
					method 1) -->
				<!-- <xsl:sort data-type="number" order="descending" select="count(key('files', 
					@name)/error)"/> -->
	...
	<xsl:template match="checkstyle" mode="summary">
		...
		<xsl:variable name="errorCount"
			select="count(file/error[@severity='error'])" />
		<xsl:variable name="warningCount"
			select="count(file/error[@severity='warning'])" />
		<xsl:variable name="infoCount"
			select="count(file/error[@severity='info'])" />
		<table class="log" border="0" cellpadding="5" cellspacing="2"
			width="100%">
			<tr>
				<th>Files</th>
